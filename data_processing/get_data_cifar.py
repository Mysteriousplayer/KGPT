...
for iter_dico in range(100):
    ...
    iter_herding = 0
    iter_herding_eff = 0
    while not (np.sum(alpha_dr_herding[iter_dico, :] != 0) == min(500, 500)) and iter_herding_eff < 1000:
        tmp_t = np.dot(w_t, D)
...
fname = '{}_cifar_{}.npy'.format(args.task, args.shot)
