...
# ADDED
    def forward_attention(self, x: torch.Tensor):
        ...
        return x[:, 0, 1:]
# ADDED
    def forward_attention_weights(self, x: torch.Tensor):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat([self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)  # shape = [*, grid ** 2 + 1, width]
...
class CLIP(nn.Module):
    ...
    def encode_image_attention_weights(self, image):
        return self.visual.forward_attention_weights(image.type(self.dtype))
