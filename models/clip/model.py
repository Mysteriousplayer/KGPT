...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, instance_tokens=None, class_tokens=None):
        if instance_tokens is None and class_tokens is None:
            return self.resblocks(x)
        elif instance_tokens is not None and class_tokens is None:
            for i, block in enumerate(self.resblocks):
                if i > 0: #####
                    instance_token = instance_tokens[i].permute(1, 0, 2)
                    x = torch.cat([x[:1, :, :], instance_token, x[1 + instance_token.shape[0]:, :, :]], dim=0)
                    x = block(x)
                else:
                    x = block(x)
            return x
            ...
            for i, block in enumerate(self.resblocks):
                if i > 0: #####
                    # print(i)
                    instance_token = instance_tokens[i].permute(1, 0, 2)
                    class_token = class_tokens[i].permute(1, 0, 2)
                    x = torch.cat([x[:1 + instance_token.shape[0], :, :], class_token, x[1 + instance_token.shape[0] + class_token.shape[0]:, :, :]], dim=0)
                    x = torch.cat([x[:1, :, :], instance_token, x[1 + instance_token.shape[0]:, :, :]], dim=0)
                    x = block(x)
                else:
                    x = block(x)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, instance_tokens=None, class_tokens=None, new_class_embedding=None):
        ...
        if new_class_embedding is None:
            ...
            x = torch.cat([new_class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)

        instance_tokens_l0, class_tokens_l0 = None, None
        if instance_tokens is not None:
            map_instance_tokens = torch.zeros(instance_tokens.shape[0], x.shape[0], instance_tokens.shape[1], x.shape[-1], dtype=x.dtype, device=x.device)
        ...
        for i in range(self.layers): #self.layers
            ...
            if class_tokens is not None:
                ...
                map_class_tokens[i] = class_tokens[i].to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device)
                if i == 0:
                    class_tokens_l0 = map_class_tokens[i]

        x = x + self.positional_embedding.to(x.dtype)

        if class_tokens is not None:
            x = torch.cat([x[:,:1,:], class_tokens_l0, x[:,1:,:]], dim=1)
        if instance_tokens is not None:
        ...
        if instance_tokens is None and class_tokens is None:
            x = self.transformer(x, instance_tokens, class_tokens)
        elif instance_tokens is not None and class_tokens is None: ########
            x = self.transformer(x, instance_tokens=map_instance_tokens, class_tokens=None)
        else:
    ...
    def forward_attention(self, x: torch.Tensor, instance_tokens=None, class_tokens=None, new_class_embedding=None):
        ...
        if new_class_embedding is None:
            ...
            x = torch.cat([new_class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype,
        ...
        if class_tokens is not None:
            map_class_tokens = torch.zeros(class_tokens.shape[0], x.shape[0], class_tokens.shape[2], x.shape[-1],
        ...
        for i in range(self.layers):  # self.layers
            if instance_tokens is not None:
                ...
                map_instance_tokens[i] = instance_tokens[i].to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1],
                                                                                      dtype=x.dtype, device=x.device)
                if i == 0:
                    instance_tokens_l0 = map_instance_tokens[i]
            if class_tokens is not None:

                map_class_tokens[i] = class_tokens[i].to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1],
                                                                                dtype=x.dtype, device=x.device)
                if i == 0:
                    class_tokens_l0 = map_class_tokens[i]
        x = x + self.positional_embedding.to(x.dtype)
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND
        if instance_tokens is None and class_tokens is None:
            x = self.transformer.forward_attention(x, instance_tokens, class_tokens)
        elif instance_tokens is not None and class_tokens is None:
            x = self.transformer.forward_attention(x, instance_tokens=map_instance_tokens, class_tokens=None)
        else:
            x = self.transformer.forward_attention(x, map_instance_tokens, map_class_tokens)
        print(x.shape)
        return x[:, 0, 1 + class_tokens_l0.shape[-2] + instance_tokens_l0.shape[-2]:]
